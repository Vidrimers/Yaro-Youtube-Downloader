...
Telegram бот для получения прямых ссылок на скачивание видео с YouTube в различных качествах. Бот анализирует видео через **yt-dlp**, предоставляет пользователю выбор качества через inline кнопки и:
- Для комбинированных форматов (видео+аудио) - возвращает прямую ссылку
- Для раздельных форматов (только видео) - скачивает, объединяет через **ffmpeg** и загружает готовое видео в Telegram

### ✨ Преимущества

- 🚀 **Быстрая работа** - прямые ссылки для комбинированных форматов
- 🎬 **Высокое качество** - поддержка форматов с раздельными потоками (1080p+)
- 💾 **Умное управление** - автоматическая очистка временных файлов
- 📤 **Загрузка в Telegram** - готовые видео файлы прямо в чат
...
4. При нажатии на кнопку:
   - Если формат комбинированный (видео+аудио) - бот отправляет прямую ссылку
   - Если формат раздельный (только видео) - бот скачивает видео и аудио, объединяет через ffmpeg и загружает в Telegram
5. Пользователь получает либо прямую ссылку, либо готовый видео файл
...
- **yt-dlp** - получение информации о видео и скачивание
...
### 3. Установка ffmpeg (для объединения видео и аудио)
...
**Windows:**
```bash
# Скачать ffmpeg с https://ffmpeg.org/download.html
...
2. Бот отправит сообщение с информацией о видео и кнопками выбора качества:
   ...
   [🎬 1080p MP4 (250 MB)]      ← Прямая ссылка
   [🔄 🎬 1080p MP4 (300 MB)]   ← Скачивание + объединение
   [🎬 720p MP4 (120 MB)]       ← Прямая ссылка
   [🎬 480p MP4 (60 MB)]        ← Прямая ссылка
   ```
   
   🔄 - индикатор что формат требует объединения видео и аудио

3. Нажмите на кнопку с нужным качеством

4. Бот либо отправит прямую ссылку:
   ```
   ✅ Ссылка готова!
   Качество: 720p
   ...
   Либо скачает, объединит и загрузит видео:
   ```
   ⏬ Скачиваю видео...
   🎵 Скачиваю аудио...
   🔄 Объединяю видео и аудио...
   ⏫ Загружаю в Telegram...
   
   [Готовое видео файлом в чате]
   ```
...
│   ├── fileManager.js    # Модуль работы с файлами и ffmpeg
...
# Установка ffmpeg, python и yt-dlp
...
3. **Ограничения Telegram** - максимальный размер файла 2GB
4. **Rate limits YouTube** - при большой нагрузке
5. **Требуется место на диске** - для временных файлов при объединении видео
